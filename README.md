...
```
# EXAMPLE: Copy the latest official FOSSCAD library repo to a local file, for distribution purposes
...
mv fosscad-repo ${FOLDER_NAME}
# make tar+xz archive (Linux)
tar --numeric-owner --owner=0 --group=0 --sort=name --mtime=$TAR_TIMESTAMP -cv ${FOLDER_NAME} | xz -zc -7e > ${FOLDER_NAME}.tar.xz
touch -d $TAR_TIMESTAMP ${FOLDER_NAME}.tar.xz
# make zip archive (Windows)
find ${FOLDER_NAME}/ -exec touch -d $TAR_TIMESTAMP {} +
zip -r ${FOLDER_NAME}.zip ${FOLDER_NAME}
touch -d $TAR_TIMESTAMP ${FOLDER_NAME}.zip
```
